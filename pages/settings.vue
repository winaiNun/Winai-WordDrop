  ...
  <div class="min-h-screen font-mali">
    ...
    <main class="max-w-3xl mx-auto px-4 py-5 flex flex-col gap-5 pb-12">
      ...
      <section class="card">
        ...
        <div class="flex flex-wrap gap-3">
          ...
          <input
            ref="excelFallbackInput"
            type="file"
            accept=".xlsx,.xls,.csv"
            class="hidden"
            @change="onExcelFallback"
          />
        </div>
        <!-- Import-replaces note -->
        ...
        <div class="mt-4 bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-800">
          <p class="font-bold mb-1">คอลัมน์ใน Excel Template</p>
          <div class="grid grid-cols-2 gap-x-4 gap-y-0.5">
            <span>📝 <strong>word</strong> — APPLE (A-Z)</span>
      ...
      <section class="card">
        ...
        <!-- Loading progress -->
        <div v-if="imgStore.isLoading.value" class="my-2 px-3 py-2 bg-blue-50 border border-blue-200 rounded-xl text-sm text-blue-700 font-bold">
        ...
        <div class="flex flex-wrap gap-3">
          ...
          <button
            v-if="storedImageCount > 0"
            class="action-btn bg-rose-400 hover:bg-rose-500"
          ...
          <input
            ...
            multiple
            class="hidden"
            @change="onFolderFallback"
          />
        </div>

        <!-- Browser support note -->
        <p v-if="!supportsFilePicker" class="mt-2 text-xs text-orange-600">
          ⚠️ เบราว์เซอร์นี้ไม่รองรับ File System API — ใช้ Chrome หรือ Edge เพื่อประสบการณ์ที่ดีที่สุด
        </p>
      </section>
        ...
        <div class="overflow-hidden rounded-xl border border-purple-100">
          <table class="w-full text-sm">
            <thead class="bg-purple-50 border-b border-purple-100">
              <tr>
                <th class="px-4 py-2 text-left text-purple-700 font-bold">#</th>
                <th class="px-4 py-2 text-left text-purple-700 font-bold">คำศัพท์</th>
                <th class="px-4 py-2 text-left text-purple-700 font-bold">Emoji</th>
                <th class="px-4 py-2 text-left text-purple-700 font-bold hidden sm:table-cell">รูปภาพ</th>
                <th class="px-4 py-2 text-left text-purple-700 font-bold">หมวดหมู่</th>
                <th class="px-4 py-2 text-center text-purple-700 font-bold">จัดการ</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="(item, idx) in filteredList" :key="item.id"
                ...
                <td class="px-4 py-2 font-bold text-gray-800 tracking-widest">{{ item.word }}</td>
                <td class="px-4 py-2 text-2xl">{{ item.emoji || '—' }}</td>
                <td class="px-4 py-2 text-gray-500 text-xs hidden sm:table-cell max-w-[140px] truncate">{{ item.image || '—' }}</td>
                <td class="px-4 py-2">
    ...
    <Transition name="modal">
      <div v-if="confirmMsg" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div class="absolute inset-0 bg-black/40 backdrop-blur-sm" @click="confirmMsg = ''" />
...
// ── Vocab ─────────────────────────────────────────────────────────────────────
const {
  vocabList, filteredVocabList, categories, categoryCounts, activeCats,
  topic, playMode, init,
  setTopic, setPlayMode, isCatActive, toggleCategory, selectAllCategories, deselectAllCategories,
  add, update, remove, reset, replaceAll,
} = useVocab()

const activeWordCount  = computed(() => filteredVocabList.value.length)
// Plain object unwrap so template can use catCount['ผลไม้'] safely
const catCount         = computed(() => categoryCounts.value)
onMounted(async () => { init(); await syncCount() })

// ── Topic & play mode (local mirrors for v-model) ─────────────────────────────
...
function changePlayMode(m: 'random' | 'sequence') {
  localPlayMode.value = m
  setPlayMode(m)
}
...
async function downloadTemplate() {
  ...
  const data = [
    ['word','emoji','image','category'],
    ...vocabList.value.map(i => [i.word, i.emoji ?? '', i.image, i.category]),
  ]
  ...
  wsD['!cols'] = [{ wch: 14 }, { wch: 10 }, { wch: 22 }, { wch: 16 }]
...
const form = ref({ word: '', emoji: '', image: '', category: '' })

function openAddModal() {
  editingId.value = null
  form.value = { word: '', emoji: '', image: '', category: categories.value[0] ?? '' }
  showModal.value = true
}
function openEditModal(item: VocabItem) {
  editingId.value = item.id
  form.value = { word: item.word, emoji: item.emoji ?? '', image: item.image, category: item.category }
  showModal.value = true
}
function saveItem() {
  const p = { word: form.value.word.trim(), emoji: form.value.emoji.trim(), image: form.value.image.trim(), category: form.value.category.trim() }
...
function confirmClearVocab() {
  confirmMsg.value = 'ล้างข้อมูลคำศัพท์ทั้งหมด? (พร้อมนำเข้า Excel ใหม่)'
  _pending = () => { replaceAll([]); showToast('🗑️ ล้างข้อมูลคำศัพท์แล้ว') }
}
